...
class Knight(Piece):
    ...
    def piece_movement(self, tabletop, dest_x, dest_y):
        self.pos_x = dest_x
        self.pos_y = dest_y
        player_knight = tabletop.tabletop[str(self.pos_x) + str(self.pos_y)]
        tabletop.tabletop[str(dest_x) + str(dest_y)] = player_knight
    ...
    def check_movement(self, tabletop, dest_x, dest_y):
        if tabletop.tabletop[str(dest_x) + str(dest_y)] is not None and\
            ...
            return False
        elif (dest_x == self.pos_x - 2 and dest_y == self.pos_y + 1) or\
                (dest_x == self.pos_x - 2 and dest_y == self.pos_y - 1):
            Printer.printer("Valid_movement")
            return True
        elif (dest_x == self.pos_x - 1 and dest_y == self.pos_y + 2) or\
                (dest_x == self.pos_x - 1 and dest_y == self.pos_y - 2):
            Printer.printer("Valid_movement")
            return True
        elif (dest_x == self.pos_x + 1 and dest_y == self.pos_y + 2) or\
                (dest_x == self.pos_x + 1 and dest_y == self.pos_y - 2):
